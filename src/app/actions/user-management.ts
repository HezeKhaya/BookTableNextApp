...
export async function updateUserPhone(userIdToEdit: string, newPhone: string, adminUserId: string) {
    const supabase = await createClient();
    if (!adminUserId || !userIdToEdit) {
        return { error: 'Invalid user context' };
    }
    ...
    if (adminError || !adminUser || adminUser.role_id !== 3) {
        return { error: 'Unauthorized: Only System Admins can modify users.' };
    }
    ...
    if (updateError) {
        ...
        return { error: 'A database error occurred while updating the phone number.' };
    }
